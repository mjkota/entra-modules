function ConvertFrom-ApplicationDisplayNameToGUID {
    ...
    Foreach ($ApplicationDisplayName In $ApplicationDisplayNames) {
        ...
        Switch ($ApplicationDisplayName.ToString().ToLower()){
            ...
            #"MicrosoftAdminPortals" is the represetation of the "Microsoft Admin Portal " Object in the GUI.
            "MicrosoftAdminPortals" {
                $DontSearchGraph += 1
                $ApplicationGuids += "MicrosoftAdminPortals"; Break
            ...
            #"None" is the represetation no selected cloudapps. Static Converted is required as no (single) object exists in the directory.
            "None" {
                $DontSearchGraph += 1
                $ApplicationGuids += "None"; Break
                
            }
        }
        #If none of the staticly Converted GUIDs are triggered, search in Graph. 
        If (!$DontSearchGraph){
            $URI = "https://graph.microsoft.com/v1.0/servicePrincipals?" + '$filter' + "=displayName eq '$ApplicationDisplayName'"

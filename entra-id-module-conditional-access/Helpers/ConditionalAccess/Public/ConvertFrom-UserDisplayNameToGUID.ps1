function ConvertFrom-UserDisplayNameToGUID {
  ...
  Foreach ($UserDisplayName in $UserDisplayNames) {
    ...
    if ($UserDisplayName.ToString().ToLower() -eq "all") {
      $UserGuids = $null
      $UserGuids += "All"
    }
    elseif ($UserDisplayName.ToString() -eq 'GuestsOrExternalUsers') {
      $UserGuids = $null
      $UserGuids += "GuestsOrExternalUsers"
    }
    elseif ($UserDisplayName.ToString() -eq 'None') {
      $UserGuids = $null
      $ApplicationGuids += "None"
    }

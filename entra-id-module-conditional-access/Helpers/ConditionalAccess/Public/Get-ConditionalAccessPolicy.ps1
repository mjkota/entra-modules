function Get-ConditionalAccessPolicy {
    ...
    if (($conditionalAccessPolicyResponse | Get-Member -Type NoteProperty).name -contains 'id') {
        [Array]$Policies = $conditionalAccessPolicyResponse    
    }
    else {
    ...
    If ($ConvertGUIDs -eq $True) {
        ...
        Foreach ($Policy in $Policies) {
            Write-Debug "Processing policy '$($policy.displayName)'"
            ...
            [array]$AgreementDisplayNames = ConvertFrom-AgreementGUIDToDisplayName -AgreementGuids ($Policy.grantControls.termsOfUse) -AccessToken $AccessToken

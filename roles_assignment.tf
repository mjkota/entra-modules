...
# Create app roles
resource "azuread_application_app_role" "saml_roles" {
  count               = length(local.create_app_roles)
  application_id      =  data.azuread_application.saml_app_data.id
...
resource "azuread_app_role_assignment" "group_assignments" {
  ...
  resource_object_id  = azuread_service_principal.saml_sp.object_id
  depends_on          = [ azuread_service_principal.saml_sp, azuread_group.create_groups]
}
